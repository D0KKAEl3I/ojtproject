...
export default function AcceptWorkRequest({ navigation, route, ...props }) {
	...
	const submit = () => {
		Alert.alert("작업 요청 수락", "작업 요청을 수락하시겠습니까?", [
			{
				text: '취소',
				onPress: () => null,
				style: 'cancel'
	...
	const AcceptWorkRequest = async () => {
		...
		if (workResponse.ok && messageResponse.ok) {
			Alert.alert('작업 요청 수락됨', '작업 요청이 수락되었습니다.', [
				{
					text: "확인 및 뒤로가기",
			...
			Alert.alert('작업 요청 수락 실패', '작업 요청이 수락되지 않았습니다.')
			context.setOnLoading(false)
		}
	}
